    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_GnssGetGui.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: GnssGetGui.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * GnssGetGui.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * GnssGetGui.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * GnssGetGui (application created by MATLAB Compiler)
        * run_GnssGetGui.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * GnssGetGui.dll (shared library, file extension varies by 
          platform)
        * GnssGetGui.h (library header file)
    ...
    Windows:
        * GnssGetGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * GnssGetGui.xml (documentation files)
        * GnssGetGui.pdb (program database file - if DEBUG option 
          selected)
        * GnssGetGui.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * GnssGetGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GnssGetGui_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * GnssGetGui.jar (Java class files and compiled M-code)
    ...
    Windows:
        * GnssGetGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GnssGetGui_1_0.dll (COM component)
        * GnssGetGui.xla (optionally created and placed in
          C:\Users\acer\Desktop\dsk_exe\GNSS_GET_GUI\GnssGetGui\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
