...
extern mclComponentData __MCC_GnssPlotGui_component_data;
 ...
 */
#ifndef LIB_GnssPlotGui_C_API 
#define LIB_GnssPlotGui_C_API /* No special import/export declaration */
#endif

LIB_GnssPlotGui_C_API 
bool MW_CALL_CONV GnssPlotGuiInitializeWithHandlers(
...
)
{
  ...
  if (!mclInitializeComponentInstanceWithEmbeddedCTF(&_mcr_inst,
                                                     &__MCC_GnssPlotGui_component_data,
                                                     true, NoObjectType,
                                                     ExeTarget, error_handler,
                                                     print_handler, 1312285, NULL))
    return false;
  return true;
}

LIB_GnssPlotGui_C_API 
bool MW_CALL_CONV GnssPlotGuiInitialize(void)
{
  return GnssPlotGuiInitializeWithHandlers(mclDefaultErrorHandler,
                                           mclDefaultPrintHandler);
}

LIB_GnssPlotGui_C_API 
void MW_CALL_CONV GnssPlotGuiTerminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_GnssPlotGui_component_data.path_to_component = path_to_component; 
  if (!GnssPlotGuiInitialize()) {
    return -1;
  }
  argc = mclSetCmdLineUserData(mclGetID(_mcr_inst), argc, argv);
  _retval = mclMain(_mcr_inst, argc, argv, "gnssplot", 0);
  if (_retval == 0 /* no error */) mclWaitForFiguresToDie(NULL);
  GnssPlotGuiTerminate();
#if defined( _MSC_VER)
  PostQuitMessage(0);
#endif
...
#endif
{
  if (!mclInitializeApplication(
    __MCC_GnssPlotGui_component_data.runtime_options,
    __MCC_GnssPlotGui_component_data.runtime_option_count))
    return 0;
