    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_GnssPlotGui.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: GnssPlotGui.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * GnssPlotGui.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * GnssPlotGui.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * GnssPlotGui (application created by MATLAB Compiler)
        * run_GnssPlotGui.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * GnssPlotGui.dll (shared library, file extension varies by 
          platform)
        * GnssPlotGui.h (library header file)
    ...
    Windows:
        * GnssPlotGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * GnssPlotGui.xml (documentation files)
        * GnssPlotGui.pdb (program database file - if DEBUG option 
          selected)
        * GnssPlotGui.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * GnssPlotGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GnssPlotGui_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * GnssPlotGui.jar (Java class files and compiled M-code)
    ...
    Windows:
        * GnssPlotGui.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * GnssPlotGui_1_0.dll (COM component)
        * GnssPlotGui.xla (optionally created and placed in
          C:\Users\acer\Desktop\dsk_exe\GNSS_PLOT_GUI\GnssPlotGui\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
